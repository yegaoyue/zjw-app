<template>
	<view class="box">
		...
		<view v-if="sfcState === 3" class="box_content">
			<text class="title">打款凭证</text>
			<view style="padding: 10rpx;">
				<imageUp :displayUp="false" :imageUrl="moneyData.sfcUpload"></imageUp>
			</view>
		</view>
		...
		<view v-if="sfcState === 3" class="box_content">
			<view class="item_box">
				<view class="item">
					<text class="name">收款备注</text>
					<input class="value" type="text" v-model="moneyData.dpRemark" placeholder="请输入备注信息" />
				</view>
			</view>
		</view>

		<view v-if="sfcState === 3" style="background-color: #E6A23C;padding: 20rpx 30rpx;text-align: center;border-radius: 10rpx;"
		 @click="confirmedPaid">
			<text style="color: #fff;font-size: 32rpx;">确认收款</text>
		</view>
	</view>
</template>
...
<script>
	import {
		getApplyFinanceEntrusted,
		getFacilitatorInfoById,
		confirmCollectionEntrusted
	...
	export default {
		...
		methods: {
			// 结单数量
			getApplyFinanceEntrusted() {
				getApplyFinanceEntrusted(this.checkForm).then(res => {
				...
				if (this.moneyData.dpRemark === '') {
					...
					uni.showModal({
						...
						success: function(res) {
							if (res.confirm) {
								confirmCollectionEntrusted({
									sfcId: this.moneyData.sfcId, //账单id
