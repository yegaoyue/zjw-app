		...
		<scroll-view scroll-y="true" class="scroll-Y">
			<view class="box">
				<view class="list " v-for="item in list" @click="details(item)">
					<view class="left">
						<image :src="'../../../static/images/'+(item.sfcState === 3 ? '07.png' : item.sfcState === 2 ? '04.png': '')"></image>
					</view>
					<view class="right">
						...
						<view class="right_bottom">
							...
							<text :style="'background-color:' + (item.sfcState === 3 ? '#2a92be;' : item.sfcState === 2 ? '#c2b81e;' : '' )">{{item.sfcState === 3 ? '待收款' : item.sfcState === 2 ? '未结清' : ''}}</text>
						</view>
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			details(val) {
				...
				uni.navigateTo({
					url: `./receivePaymentDetails?str=${str}`
				})
